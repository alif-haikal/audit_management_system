<script type="text/javascript">
    let baseUrl = "{{ route('modelhasrole.index') }}";
    let createUrl = "{{ route('modelhasrole.create') }}";

    $(document).ready(function () {
        $('#modelHasRoleDatatable').DataTable({
            'scrollX': true,
            'scrollY': '500px',
            'scrollCollapse': true,
            ...
            columnDefs: [{
                ...
                "render": function(id, type, full, meta) {
                    let showUrl = "{{ route('modelhasrole.show', 'data-id') }}";                    
                    let editUrl = "{{ route('modelhasrole.edit', 'data-id') }}";
                    let deleteUrl = "{{ route('modelhasrole.destroy', 'data-id') }}";

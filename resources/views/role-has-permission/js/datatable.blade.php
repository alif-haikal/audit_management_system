<script type="text/javascript">
    let baseUrl = "{{ route('rolehaspermission.index') }}";
    let createUrl = "{{ route('rolehaspermission.create') }}";

    $(document).ready(function () {
        $('#roleHasPermissionDatatable').DataTable({
            'scrollX': true,
            'scrollY': '500px',
            'scrollCollapse': true,
            ...
            columnDefs: [{
                ...
                "render": function(id, type, full, meta) {
                    let showUrl = "{{ route('rolehaspermission.show', 'data-id') }}";                    
                    let editUrl = "{{ route('rolehaspermission.edit', 'data-id') }}";
                    let deleteUrl = "{{ route('rolehaspermission.destroy', 'data-id') }}";

<script type="text/javascript">
    let baseUrl = "{{ route('objective.index') }}";
    let createUrl = "{{ route('objective.create') }}";

    $(document).ready(function () {
        $('#objectiveDatatable').DataTable({
            'scrollX': true,
            'scrollY': '500px',
            'scrollCollapse': true,
            ...
            columnDefs: [{
                ...
                "render": function(id, type, full, meta) {
                    let showUrl = "{{ route('objective.show', 'data-id') }}";                    
                    let editUrl = "{{ route('objective.edit', 'data-id') }}";
                    let deleteUrl = "{{ route('objective.destroy', 'data-id') }}";

<script type="text/javascript">
    let baseUrl = "{{ route('modelhaspermission.index') }}";
    let createUrl = "{{ route('modelhaspermission.create') }}";

    $(document).ready(function () {
        $('#modelHasPermissionDatatable').DataTable({
            'scrollX': true,
            'scrollY': '500px',
            'scrollCollapse': true,
            ...
            columnDefs: [{
                ...
                "render": function(id, type, full, meta) {
                    let showUrl = "{{ route('modelhaspermission.show', 'data-id') }}";                    
                    let editUrl = "{{ route('modelhaspermission.edit', 'data-id') }}";
                    let deleteUrl = "{{ route('modelhaspermission.destroy', 'data-id') }}";
